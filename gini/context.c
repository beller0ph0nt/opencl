...
context_t* context_create(cl_device_type dev_type)
{
    ...
    _w_clGetPlatformIDs(0, NULL, &context->plat_count);

    context->plat = _wrp_malloc(sizeof(*context->plat) * context->plat_count);

    _w_clGetPlatformIDs(context->plat_count, context->plat, NULL);
    ...
    {
        _w_clGetDeviceIDs(context->plat[p], context->dev_type, 0, NULL, &context->dev_on_plat[p]);
        ...
        _w_clGetDeviceIDs(context->plat[p], context->dev_type, context->dev_on_plat[p], context->dev[p], NULL);
        ...
        {
            _w_clGetDeviceInfo(context->dev[p][d], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(context->dev_prop[p][d].max_comp_units), &context->dev_prop[p][d].max_comp_units, NULL);
            _w_clGetDeviceInfo(context->dev[p][d], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(context->dev_prop[p][d].glob_mem_size), &context->dev_prop[p][d].glob_mem_size, NULL);
            _w_clGetDeviceInfo(context->dev[p][d], CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(context->dev_prop[p][d].max_mem_alloc_size), &context->dev_prop[p][d].max_mem_alloc_size, NULL);

            context->total_comp_units += context->dev_prop[p][d].max_comp_units;

            context->contexts[p][d] = _w_clCreateContext(properties, DEV_PER_CONTEXT, &context->dev[p][d], NULL, NULL);
            context->cmd[p][d] = _w_clCreateCommandQueue(context->contexts[p][d], context->dev[p][d], CL_QUEUE_PROFILING_ENABLE);
        }
    }
...
void context_clear(context_t *context)
{
    ...
    for (p = 0; p < context->plat_count; p++)
    {
        for (d = 0; d < context->dev_on_plat[p]; d++)
        {
            _w_clReleaseCommandQueue(context->cmd[p][d]);
            _w_clReleaseContext(context->contexts[p][d]);
        }
    }
